            ...
            <div className="container mx-auto px-4 py-4 space-y-1">
              ...
              {!user ? (
                <div className="pt-4 mt-4 space-y-3 border-t border-white/5">
                  <Link
                    to="/login"
                    className="block px-4 py-3 text-[15px] font-medium text-purple-100 hover:text-white transition-colors rounded-lg hover:bg-white/5"
                    onClick={() => setIsMobileMenuOpen(false)}
                  >
                    Log in
                  </Link>
                  <Link
                    to="/signup"
                    className="block w-full px-4 py-3 bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-lg text-center text-[15px] font-semibold shadow-lg hover:shadow-xl hover:shadow-purple-500/25 hover:scale-105 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-purple-500/50 focus:ring-offset-2 focus:ring-offset-[#181C2A]"
                    onClick={() => setIsMobileMenuOpen(false)}
                  >
                    Sign up
                  </Link>
                ...
                <div className="pt-4 mt-4 space-y-3 border-t border-white/5">
                  <div className="flex items-center bg-white/10 border border-white/20 rounded-full px-5 py-2 shadow-lg mb-2">
                    <span className="font-semibold text-white mr-3 flex items-center">
                      <FaUser className="w-4 h-4 mr-1 text-cyan-300" />
                      {user.name}
                    </span>
                  ...
                  <Link
                    to="/dashboard"
                    className="block px-4 py-2 text-purple-100 hover:bg-white/10 transition-colors rounded-lg"
                    onClick={() => setIsMobileMenuOpen(false)}
                  >
                    <FaTachometerAlt className="inline w-4 h-4 mr-2" /> Dashboard
                  </Link>
                  <Link
                    to="/dashboard/settings"
                    className="block px-4 py-2 text-purple-100 hover:bg-white/10 transition-colors rounded-lg"
                    onClick={() => setIsMobileMenuOpen(false)}
                  >
                    <FaCog className="inline w-4 h-4 mr-2" /> Settings
                  </Link>
                  <button
                    onClick={() => { handleSignOut(); setIsMobileMenuOpen(false); }}
                    className="w-full flex items-center px-4 py-2 text-purple-100 hover:bg-white/10 transition-colors rounded-lg"
                  >
